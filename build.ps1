...
# The disk's version number. Bump it here -- the pre-intro menu draws it under the
# title together with the build stamp generated below, so a disk always says what
# it is without anyone having to remember.
$version = "1.1"
...
if (Test-Path "orig\MEMLIST.BIN") {
    ...
    # ... and a handful of GAME sounds for the pre-intro menu's test key. They have
    # to be baked into THIS xex: the menu runs before any game data exists on the
    # machine. They go in the dead tail of the last music VRAM bank.
    python "tools\gen_test_sfx.py" | Select-Object -Last 9
    if ($LASTEXITCODE -ne 0) { throw "gen_test_sfx.py FAILED" }
    ...
    foreach ($f in @("out\intro_playlist.bin", "out\intro_poly.bin", "out\intro_sfx.bin",
                     "src\aw_sfx_tables.inc", "out\test_sfx.bin", "src\aw_test_sfx.inc",
                     "src\aw_test_sfx_data.inc")) {
        if (-not (Test-Path $f)) { throw "$f missing and orig/ is gone -- restore orig/ to rebuild it" }
    }
}

# --- version line : "V<version>  <date> <time>", drawn under the title ------------
# Generated BEFORE pass 1 and not touched again: the two intro passes must assemble
# to the same byte size (GAME_SEC depends on it), and a stamp that moved between them
# would be a different string length waiting to happen.
$verStr = "V$version  " + (Get-Date -Format "yyyy-MM-dd HH:mm")
$verX = [math]::Floor((40 - $verStr.Length) / 2)
Set-Content -Path "src\aw_version.inc" -Encoding ascii -Value @(
    "; auto-generated by build.ps1 - DO NOT EDIT",
    "; the version + build stamp line on the pre-intro menu (src/aw_settings.asm)",
    "s_ver   dta c'$verStr',0",
    "VER_X   = $verX")
Write-Host "[intro] version line: $verStr"
...
# ... and the OTHER half of the covox story: the pre-intro menu (src/aw_settings.asm)
# lets the user say WHERE their card is, and cv_set_base rewrites the operand of every
# DAC store. A missing entry in that table assembles and runs fine on $D280 and
# silently splits the output on every other base, so run it for all four bases and
...
python "tools\verify_covox_base.py" "awgame.xex" "out\awgame.lst" | Select-Object -Last 1
if ($LASTEXITCODE -ne 0) { throw "verify_covox_base.py FAILED on the game - a chosen covox base does not reach the DAC" }
